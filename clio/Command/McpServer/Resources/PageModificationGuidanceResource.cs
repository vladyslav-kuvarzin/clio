...
public sealed class PageModificationGuidanceResource {
	...
	internal static readonly TextResourceContents Guide = new() {
		...
		Text = """
		       ...
		       | working with a dashboard page (a page inherited from `BaseDashboardTemplate`), or adding/arranging/sizing/styling any analytics widget (metric/indicator or chart) on it | `dashboards`, `analytics-widgets` | Mandatory read before composing the body — owns the canonical dashboard layout, widget placement, sizing, and styling. The `dashboards` guide routes to `indicator-widget` + `get-component-info` for a widget's runtime contract. |
		       | designing or laying out a page's UI/UX (choosing a component for a concept, placing/ordering fields, grid columns/colSpan, container nesting, grouping into tabs/groups/profile islands, lookup dropdown-vs-window, captions/tooltips/placeholders), OR reviewing/auditing a page's design, copy, or accessibility | `ui-guidelines` | Native-looking, gap-free, accessible Freedom UI follows the design rules the index owns (concept-to-component map, column math, contrast, copy); it routes on to its layout, accessibility, and review leaves. Author and review from it, not from memory. ALWAYS read the existing page style first (get-page) so new inputs match the existing label positions/spacing, and do not append a trailing period to every short field description. |
